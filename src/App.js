...
class App extends Component {
  ...
  clickImg = id => {
    ...
    if (activeImgId === id) {
      this.setState(prevState => ({
        ...
        activeImgId: imagesList[randomNum].id,
      }))
    } else {
  ...
  render() {
    const {score, timer, activeTabId, activeImgId} = this.state
    const activeTabIdImgsList = imagesList.filter(
      eachItem => eachItem.category === activeTabId,
    )
    const activeImgList = imagesList.filter(
          ...
          {timer === 0 ? (
            ...
            <div className="inner-card-container">
              ...
              <ul className="images-container">
                {activeTabIdImgsList.map(eachItem => (
                  <ImgItem
                    eachItem={eachItem}
                    key={eachItem.id}
